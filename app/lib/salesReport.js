...
// Sales report generation, shared by three callers:
//   /api/report              - the owner presses "Generate Daily Report"
//   /api/cron/daily-report   - the scheduled 11 PM email/WhatsApp
//   /api/cron/monthly-report - reuses only the category map for its PDF
//
// This lived in all three routes as near-identical copies, which meant a change
// to the report format reached the screen but not the email. One copy now.

export function buildItemCategoryMap() {
  const map = {};
